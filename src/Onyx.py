...
def calc_VP(scaled_freq_TS_natural, scaled_freq_TS_isotope, scaled_freq_GS_natural, scaled_freq_GS_isotope):
    """
...
def calc_ZPE_BM(sum_u_GS_natural, sum_u_TS_natural, sum_u_GS_isotope, sum_u_TS_isotope):
    """
...
def calc_EXC(prod_GS_unlabaeled, prod_TS_unlabaeled, prod_GS_labaeled, prod_TS_labaeled):
    """
...
def calc_IE_rigid_rotor_method(GS_unlabeled, GS_labeled, TS_unlabeled, TS_labeled, temp, scale_factor, count):
    """
        Input: 4 log files/output file from freqchk, temperature, \
                scaling factor, and the calculation number
...
def parse_file_names(directory):
    ...
    log_files = [str(file_name) for file_name in os.listdir(
        directory.get()) if file_name.endswith('log')]
...
def no_input_LOG():
    ...
    ERROR_OUT.write("======================Onyx Input File======================\n\n" +
                    ...
                    "=====================USER ENTRY BELOW======================\n\n")
    ERROR_OUT.write(
        "Unlabeled transition state (or state 2) file(s):" + "\n\n")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="A program to " +
                                     ...
                                     "'python Onyx.py'.")
    parser.add_argument('-fp', '--freqchkprint', help='print input file for freqchk functionality. ' +
    ...
    parser.add_argument('-fi', '--freqchkinput', dest="fchktxtname", help='Calculate IE utilizing freqchk functionality. Pass in filled ' +
                        'out text input file (generated by -fi option)')
    parser.add_argument('-li', '--loginput', dest="logtxtname", help='Calculate IE utilizing log file functionality. Pass in filled ' +
